LANGUAGE_CHOOSER = "Выбери язык / Choose a language"

TEXTS = {
    "ru": {
        "intro": "Я помогаю двум людям спокойно разобрать конфликт: понять, что произошло, где расхождения и как двигаться дальше.",
        "unknown_command": "Не знаю такой команды. Вот что я умею:\n\n/newcase — создать новое обсуждение\n/join CODE — присоединиться по коду\n/mycases — посмотреть мои обсуждения\n/case CODE — открыть обсуждение\n/feedback — оставить отзыв\n/start — открыть главное меню",
        "newcase_prompt": "Опиши конфликт одним предложением. Это будет название обсуждения.",
        "invite_title": "Приглашение в обсуждение конфликта:",
        "invite_link": "Ссылка для второго участника:",
        "invite_forward": "Можно просто переслать это сообщение собеседнику.",
        "your_side_intro": "А пока можешь сразу ответить на вопросы по своей стороне конфликта.",
        "join_usage": "Использование: /join CODE",
        "case_not_found": "Обсуждение с таким кодом не найдено.",
        "case_already_joined": "К этому обсуждению уже присоединился второй участник.",
        "joined_intro": "Ты присоединился к обсуждению. Ниже тема, по которой будут вопросы.",
        "start_questions": "Начнём с короткого опроса.",
        "participant_joined": "Второй участник присоединился. Теперь можно продолжать по теме:",
        "answer_questions": "Ответь на вопросы.",
        "feedback_choose": "Выбери область, по которой хочешь оставить отзыв.",
        "feedback_prompt": "Коротко оставь отзыв. Лучше всего указать область: понятность вопросов / ход разговора / качество итогов / удобство / другое.",
        "case_usage": "Использование: /case CODE",
        "case_access_denied": "Обсуждение не найдено или у тебя нет к нему доступа.",
        "status": "Статус",
        "status_waiting_for_b": "Ждём второго участника",
        "status_intake": "Идёт сбор ответов",
        "status_intake_a": "Ждём ответы первой стороны",
        "status_intake_b": "Ждём ответы второй стороны",
        "status_analyzing": "Готовим итоги",
        "status_analysis_ready": "Итоги готовы",
        "status_resolved": "Удалось договориться",
        "status_paused": "Обсуждение на паузе",
        "status_continues": "Обсуждение продолжается",
        "side_a": "Позиция A",
        "side_b": "Позиция B",
        "common_ground": "Общее",
        "differences": "Расхождения",
        "options": "Варианты",
        "no_cases": "У тебя пока нет обсуждений.",
        "my_cases": "Твои обсуждения:",
        "share_prompt": "Как можно использовать этот ответ при пересказе второй стороне?",
        "share_mode_summary": "Пересказать нейтрально",
        "share_mode_private": "Не передавать другой стороне",
        "share_mode_quote": "Можно цитировать почти дословно",
        "share_mode_selected": "Для следующего ответа выбрано:",
        "feedback_area_selected": "Область обратной связи:",
        "write_comment": "Теперь напиши короткий комментарий.",
        "feedback_saved": "Спасибо. Обратную связь записал.",
        "thinking_next": "Записал. Сейчас быстро обработаю ответ и пришлю следующий вопрос.",
        "thinking_analysis": "Принял. Теперь немного подумаю над общей картиной и соберу нейтральную сводку для обеих сторон.",
        "position_saved": "Спасибо. Твоя позиция записана.",
        "feedback_nudge": "Если захочешь, можешь оставить короткий отзыв через /feedback",
        "topic": "Тема конфликта",
        "period": "Когда/период",
        "questions": [
            ("what_happened", "1/5. Что произошло? Опиши ситуацию коротко и по делу."),
            ("conflict_date", "2/5. Когда произошёл этот конфликт или в какой период он развивался? Если конфликт повторяется, так и напиши."),
            ("what_hurt", "3/5. Что тебя задело или больше всего напрягло в этой ситуации?"),
            ("desired_outcome", "4/5. Какой результат для тебя был бы приемлемым?"),
            ("boundary", "5/5. Что для тебя точно неприемлемо или где проходит граница?"),
        ],
        "feedback_areas": {
            "questions": "Корректность вопросов",
            "flow": "Ход разговора",
            "summary": "Качество итогов",
            "ux": "Удобство",
            "other": "Другое",
        },
        "lang_set": "Язык переключён на русский.",
        "menu_create": "Создать обсуждение",
        "menu_list": "Мои обсуждения",
        "menu_feedback": "Оставить отзыв",
        "menu_language": "Сменить язык",
        "open_discussion": "Открыть",
        "discussion_invite": "Приглашение",
        "discussion_delete": "Удалить",
        "discussion_continue": "Продолжить",
        "discussion_feedback": "Отзыв об обсуждении",
        "discussion_deleted": "Обсуждение удалено.",
        "delete_confirm": "Точно удалить это обсуждение?",
        "delete_yes": "Да, удалить",
        "delete_no": "Нет, оставить",
        "code_label": "Код",
        "outside_text": "Сейчас я не жду обычный ответ. Выбери действие в меню.",
        "decision_prompt": "Какой следующий шаг ближе тебе по итогам этого обсуждения?",
        "decision_resolve": "Удалось договориться",
        "decision_continue": "Продолжить разбор",
        "decision_pause": "Поставить на паузу",
        "decision_saved": "Твой выбор записан.",
        "decision_wait_other": "Теперь ждём решение второй стороны.",
        "decision_both_resolved": "Обе стороны отметили, что удалось договориться. Обсуждение можно считать завершённым 🤝",
        "decision_both_continue": "Обе стороны хотят продолжить разбор. Можно вернуться к обсуждению и уточнить спорные места.",
        "decision_both_pause": "Обе стороны выбрали паузу. Можно вернуться к обсуждению позже.",
        "decision_mixed": "Стороны выбрали разные следующие шаги. Лучше отдельно уточнить, что именно мешает перейти к общему решению.",
        "analysis_failed": "Не удалось подготовить итоги. Попробуй открыть обсуждение позже или запустить разбор ещё раз.",
        "next_waiting_for_b": "Сейчас нужно, чтобы второй участник перешёл по приглашению и присоединился.",
        "next_intake_a": "Сейчас нужны ответы первой стороны. Можно продолжить вопросы.",
        "next_intake_b": "Сейчас нужны ответы второй стороны. Ждём её ответы.",
        "next_intake": "Сейчас идёт сбор ответов от участников.",
        "next_analyzing": "Сейчас бот готовит итоги обсуждения.",
        "next_analysis_ready": "Итоги готовы. Можно выбрать следующий шаг по обсуждению.",
        "next_resolved": "Обсуждение завершено: стороны отметили, что удалось договориться.",
        "next_paused": "Обсуждение поставлено на паузу. К нему можно вернуться позже.",
        "next_continues": "Обсуждение продолжается. Можно вернуться к спорным местам и продолжить разбор.",
        "next_step": "Что происходит сейчас",
    },
    "en": {
        "intro": "I help two people calmly work through a conflict: understand what happened, where the differences are, and how to move forward.",
        "unknown_command": "I don't know that command. Here is what I can do:\n\n/newcase — create a new case\n/join CODE — join a case\n/mycases — list your cases\n/case CODE — view a specific case\n/feedback — leave feedback\n/start — show intro",
        "newcase_prompt": "Describe the conflict in one sentence. This will be the case title.",
        "invite_title": "Invitation to a conflict case:",
        "invite_link": "Link for the second participant:",
        "invite_forward": "You can simply forward this message to the other person.",
        "your_side_intro": "Meanwhile, you can already answer the questions from your side of the conflict.",
        "join_usage": "Usage: /join CODE",
        "case_not_found": "No case found with that code.",
        "case_already_joined": "A second participant has already joined this case.",
        "joined_intro": "You joined the case. Here is the topic the questions are about.",
        "start_questions": "Let's start with a short intake.",
        "participant_joined": "The second participant joined. You can continue on this topic:",
        "answer_questions": "Please answer the questions.",
        "feedback_choose": "Choose the area you want to leave feedback about.",
        "feedback_prompt": "Leave short feedback. Best areas: question quality / bot flow / summary quality / UX / other.",
        "case_usage": "Usage: /case CODE",
        "case_access_denied": "Case not found or you do not have access to it.",
        "status": "Status",
        "status_waiting_for_b": "Waiting for the second participant",
        "status_intake": "Collecting answers",
        "status_intake_a": "Waiting for the first side",
        "status_intake_b": "Waiting for the second side",
        "status_analyzing": "Preparing summary",
        "status_analysis_ready": "Summary ready",
        "status_resolved": "Resolved",
        "status_paused": "Paused",
        "status_continues": "Continues",
        "side_a": "Position A",
        "side_b": "Position B",
        "common_ground": "Common ground",
        "differences": "Differences",
        "options": "Options",
        "no_cases": "You do not have any cases yet.",
        "my_cases": "Your cases:",
        "share_prompt": "How may this answer be used when presenting it to the other side?",
        "share_mode_summary": "Only a neutral summary may be shared with the other side.",
        "share_mode_private": "This stays for internal bot analysis only.",
        "share_mode_quote": "This may be quoted more directly if needed.",
        "share_mode_selected": "Privacy mode for the next answer:",
        "feedback_area_selected": "Feedback area:",
        "write_comment": "Now write a short comment.",
        "feedback_saved": "Thanks. Your feedback has been saved.",
        "thinking_next": "Saved. I will quickly process this and send the next question.",
        "thinking_analysis": "Got it. Now I will think for a moment and prepare a neutral summary for both sides.",
        "position_saved": "Thanks. Your position has been saved.",
        "feedback_nudge": "If you want, you can leave short feedback via /feedback",
        "topic": "Conflict topic",
        "period": "When/period",
        "questions": [
            ("what_happened", "1/5. What happened? Describe the situation briefly and clearly."),
            ("conflict_date", "2/5. When did this conflict happen, or over what period did it develop? If it repeats, say so."),
            ("what_hurt", "3/5. What hurt you or felt most difficult in this situation?"),
            ("desired_outcome", "4/5. What outcome would be acceptable to you?"),
            ("boundary", "5/5. What is definitely unacceptable to you, or where is your boundary?"),
        ],
        "feedback_areas": {
            "questions": "Question quality",
            "flow": "Bot flow",
            "summary": "Summary quality",
            "ux": "UX / interface",
            "other": "Other",
        },
        "lang_set": "Language switched to English.",
        "menu_create": "Create discussion",
        "menu_list": "My discussions",
        "menu_feedback": "Leave feedback",
        "menu_language": "Change language",
        "open_discussion": "Open",
        "discussion_invite": "Invite",
        "discussion_delete": "Delete",
        "discussion_continue": "Continue",
        "discussion_feedback": "Feedback on discussion",
        "discussion_deleted": "Discussion deleted.",
        "delete_confirm": "Delete this discussion?",
        "delete_yes": "Yes, delete",
        "delete_no": "No, keep it",
        "code_label": "Code",
        "outside_text": "I am not waiting for a regular text right now. Choose an action from the menu.",
        "decision_prompt": "What feels like the right next step after this summary?",
        "decision_resolve": "We reached an agreement",
        "decision_continue": "Continue discussion",
        "decision_pause": "Pause for now",
        "decision_saved": "Your choice has been saved.",
        "decision_wait_other": "Now waiting for the other side.",
        "decision_both_resolved": "Both sides marked that an agreement was reached. This discussion can be considered resolved 🤝",
        "decision_both_continue": "Both sides want to continue the discussion. You can return to the unresolved points.",
        "decision_both_pause": "Both sides chose to pause. You can return later.",
        "decision_mixed": "The sides chose different next steps. It is better to clarify what prevents a shared decision.",
        "analysis_failed": "I could not prepare the summary. Try opening the discussion later or running the review again.",
        "next_waiting_for_b": "The second participant still needs to join via the invitation.",
        "next_intake_a": "The first side still needs to answer the questions.",
        "next_intake_b": "The second side still needs to answer the questions.",
        "next_intake": "The bot is still collecting answers.",
        "next_analyzing": "The bot is preparing the summary right now.",
        "next_analysis_ready": "The summary is ready. You can choose the next step.",
        "next_resolved": "The discussion is finished: both sides marked that an agreement was reached.",
        "next_paused": "The discussion is paused. You can return later.",
        "next_continues": "The discussion continues. You can return to unresolved points.",
        "next_step": "What is happening now",
    },
}
